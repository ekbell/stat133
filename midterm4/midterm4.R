...
dice_sum <- function(k=2, B=100){
  for(i in 1:B){
    B_list <- c(B_list, (sum(sample(1:6, k, replace = TRUE))))
  }
    return(B_list)
  }
...
updateDate <- function(dates, old.yr) {
  index_to_replace <- grep(old.yr, dates)
  return(gsub(old.yr, '2015', dates[index_to_replace]))
}
...
abbreviate <- function(vector, k){
  ...
  for(i in 1:length(vector)){
    new_list <- c(new_list,paste(paste(unlist(strsplit(as.character(vector[i]), ''))[1:k], sep =""), collapse = ""))
  }
  new_list <- gsub("NA+", "", new_list)
