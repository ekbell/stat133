...
name <- "Evan Bell"
github.name <- "ekbell"
email.address <- "ekbell@berkeley.edu"
...
y <- list(c(seq(from=1, to=1000, by=1)), c(seq(from=1, to=1000, by=2)), c(seq(from=1, to=1000, by=3)),
c(seq(from=1, to=1000, by=4)), c(seq(from=1, to=1000, by=5)), c(seq(from=1, to=1000, by=6)),
c(seq(from=1, to=1000, by=7)),c(seq(from=1, to=1000, by=8)),c(seq(from=1, to=1000, by=9)),
c(seq(from=1, to=1000, by=10)))
...
set.seed(344)
m <- matrix(rnorm(100, mean = 8), nrow = 10, ncol = 10)
...
cw4 <- ChickWeight[sample(1:578, 200, replace = FALSE),]
...
# 
college_mother <- infants[infants$ed == "College",]
t2 <- table(college_mother$smoke, college_mother$number)
...
abline(coef =  coef(lm(mtcars$qsec ~ mtcars$hp)))
...
plot(mtcars$wt, mtcars$disp, col = c("blue", "red"), xlab = "Car Weight", ylab = "Engine Displacement")
...
plot(WorldBank$fertility.rate, WorldBank$life.expectancy, pch = ".", col = as.numeric(WorldBank$region),
...
par(mfrow=c(1,2))
World1960 <- WorldBank[WorldBank$year == 1960,]
World2013 <- WorldBank[WorldBank$year == 2013,]
plot(World1960$fertility.rate, World1960$life.expectancy, pch = ".", col = as.numeric(World1960$region),
     xlab = "Fertility Rate", ylab = "Life Expectancy", main = "World Bank 1960")
abline(v = 2.1, col = "red")

plot(World2013$fertility.rate, World2013$life.expectancy, pch = ".", col = as.numeric(World2013$region),
     xlab = "Fertility Rate", ylab = "Life Expectancy", main = "World Bank 2013")
abline(v = 2.1, col = "red")
...
Worldida <- WorldBank[WorldBank$lending == "IDA",]
par(mfrow=c(1,1))
bin.length = seq(from = 0, to = max(7000, na.rm= T), by = 250)
hist(Worldida$GDP, breaks = bin.length)
...
diff.rain <- lapply(rain, diff)
max.diff.rain <- lapply(diff.rain, max)


# [5 pts]
# Create [prop.rain], a vector of length 5 where each entry is the 
# number of rain days (i.e. rain > 0) as a function of total days
#Note: This question does not make sense. 

rain_days <- function(rainlist){
  totaldays =0
  for(i in 1:length(rainlist)){
    if(rainlist[i] >0){totaldays = totaldays+1}
  }
  return(totaldays)
}

#rain_days_all <- lapply(rain, rain_days)
#rd <- c(2843, 1209, 787, 2338,778)
#all_days <- lapply(rain, length)
#al <- c(9878, 6751, 3959 , 11122, 3692)

prop.rain <- sapply(rain, rain_days)
...
rowsd <- function(mat){
  ...
  for(i in 1:nrow(mat)){
    sdevr <- sd(mat[i,])
    rowsdlist <- c(rowsdlist, sdevr)
  }
  return(unlist(rowsdlist))
}
...
PermDiff <- function(cases, controls, k=5000){
  allmeans <- c()
for(i in 1:k){
  allpeople <- unlist(c(cases, controls))
  new_cases <- sample(allpeople, length(cases), replace = FALSE)
  cases <- new_cases
  controls <- allpeople[allpeople != cases]
  new_mean <- mean(cases)-mean(controls)
  allmeans <- c(allmeans, new_mean)
}
  return(unlist(allmeans))
}
...
NumJackpot <- function(k, B){
  n.jackpot = c()
  for(i in 1:B){
  winning_numbers <- sort(sample(1:19, 3, replace = FALSE))
  players <- seq(0, 0, length.out = k)
  winners = 0
  for(i in 1:k){
    tickets <- sort(sample(1:19, 3, replace = FALSE))
    if(all(tickets== winning_numbers)){winners = winners+1}                     
  }
  n.jackpot = c(n.jackpot, winners)
  }
  return(unlist(n.jackpot))
}
...
match1 <- sort(unique(grep("G.T", dna), grep("G.T", dna)))
